# Handy commands:
# - `make docker-build`: builds DOCKERIMAGE (default: `selfdeco:latest`)
PROJECT ?= py3.9
...
DOCKER_OPTS := \
			...
			# -w ${WORKSPACE} \
			# -v /data:/data \
            -v /data_ssd1/BoneMeta/exp1/npy:/workspace
...
docker-start-jupyter: docker-build
	nvidia-docker run ${DOCKER_OPTS} ${DOCKER_IMAGE} \
		bash -c "jupyter notebook --port=2222 -ip=0.0.0.0 --allow-root --no-browser"
